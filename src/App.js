...
function App() {
  ...
  const [btn, setBtn] = useState({ b: "+" });

  const [inputAns, setInputAns] = useState({ a: 0 });
  ...
  return (
    <>
      ...
      <button onClick={calResetBtn} className="answerBtn">
        {btn.b}
      </button>
      <p>
        {" "}
        <input type="number" onChange={handleOnChange} value={inputAns.a} />
      </p>
      <p>{ans.answer}</p>
      <p>{correctAns.a}</p>
    </>
